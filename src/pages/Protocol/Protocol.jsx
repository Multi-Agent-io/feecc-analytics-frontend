...
function Protocol() {
  ...
  const submitDataHandler = () => {
    ...
    if (allFieldChecked && serialNumber) {
      ...
      fetch(`${conf.base_url}/api/v1/passports/${internalId}/serial?${serialBody}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${localStorage.getItem('token')}`,
        ...
        {!isSuperEngineer && protocol.status !== 'Протокол утверждён' && (
          <>
            ...
            <Button onClick={submitDataHandler}>
              {protocol.status === 'Вторая стадия испытаний пройдена' ? 'Сохранить' : 'Подтвердить и отправить'}
            </Button>
          </>
        )}
        {isSuperEngineer
  ...
  const makeGridTable = (arrayItems) => {
    const jsxArray = arrayItems.map((row, index) => {
      const {
        name = 'Без имени', value = '', deviation = '', test1 = '', test2 = '', checked = false,
      } = row;
      return (
        <>
        ...
        <div className={styles.header}>
          ...
          <div>
            ...
            <h2>
              SN:
              {' '}
              {protocol.default_serial_number !== null ? protocol.default_serial_number : '941619006-'}
              <input
