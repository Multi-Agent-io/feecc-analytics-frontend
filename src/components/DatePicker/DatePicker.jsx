...
export default function DatePicker({ onChange, value }) {
  ...
  const getDayDetails = ({
    ...
    else if (date >= numberOfDays) currentMonth = 1;
    else currentMonth = 0;

    // const  = date < 0 ? -1 : date >= numberOfDays ? 1 : 0;
  ...
  const getDateFromDateString = (dateValue) => {
    ...
    const newYear = dateData[0];
    const newMonth = dateData[1];
    const date = dateData[2];
    return { newYear, newMonth, date };
  };

  const getMonthString = (monthString) => monthMap[Math.max(Math.min(11, monthString), 0)] || 'Month';

  const getDateStringFromTimestamp = (timestamp) => {
    const dateObject = new Date(timestamp);
    const newMonth = dateObject.getMonth() + 1;
    const date = dateObject.getDate();
    return `${dateObject.getFullYear()}-${(newMonth < 10 ? `0${newMonth}` : newMonth)}-${(date < 10 ? `0${date}` : date)}`;
  };
  ...
  const onDateClick = (day) => {
    if (onChange !== undefined) onChange(day.timestamp);
  ...
  const addBackdrop = (e) => {
    if (!ref.current.contains(e.target)) changeShowDatePicker(false);
  };
  useEffect(() => {
    ...
    return (
      <div className={styles.cContainer}>
        <div className={styles.ccHead}>
          ...
          ].map((d) => <div key={d} className={styles.cchName}>{d}</div>)}
        </div>
