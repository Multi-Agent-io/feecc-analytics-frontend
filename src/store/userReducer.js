...
export const userReducer = (state = {}, action) => {
  if (action.type.startsWith('USER__')) console.log('user-reducer', action);

  switch (action.type) {
    case types.USER__AUTHORIZE: {
      return state.set('authorized', action.status);
    }
    ...
    case types.USER__FETCH_SELECTED_PASSPORT: {
      return state.set('selectedPassport', fromJS(action.passport));
    }
    ...
    case types.USER__DECODE_EMPLOYEE: {
      return state.setIn(['employees', action.userHash], action.username);
    }
    case types.USER__FETCH_PASSPORT_TYPES: {
      ...
      action.passportTypes.forEach((type) => {
        const tempType = { name: '', state: false };
        tempType.name = type;
        tempType.state = false;
        passportTypes.push(tempType);
      });
      return state.set('passportTypes', [...passportTypes]);
    }
